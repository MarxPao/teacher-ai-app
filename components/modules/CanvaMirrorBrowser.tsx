  ...
  const handleMirrorSyncToCanva = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setSyncStatus('ℹ️ Lançado em modo espelho local. Para sincronização direta em background, certifique-se de estar com a guia do Canva aberta.')
      }
    } catch {
      ...
      <div style={{ background: '#2c1a0e', padding: '10px 16px 0 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
        <div style={{ display: 'flex', gap: 6, overflowX: 'auto' }}>
          {[
            ...
            { id: 'api', label: '⚙️ Conectar API Canva Connect', icon: 'ti-api' },
          ].map(t => (
            <button
              key={t.id}
          ...
          <div style={{ padding: 24, background: '#f5efe6', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{
              ...
              <div style={{ margin: '20px 0', textAlign: 'center' }}>
                <h1 style={{ margin: '0 0 14px', fontSize: 32, fontWeight: 900, fontFamily: 'serif', lineHeight: 1.2 }}>
                  {posterTitle}
                </h1>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 20 }}>
              ...
              >
                ↗️ Exportar Direto para o Canva.com
              </button>
            </div>
          </div>
        </div>
      ...
      {activeTab === 'api' && (
        <div style={{ padding: 24, maxWidth: 640 }}>
          <h3 style={{ margin: '0 0 6px', fontSize: 18, color: '#2c1a0e', fontWeight: 800 }}>
            ⚙️ Configuração do Canva Connect (API Oficial)
          </h3>
